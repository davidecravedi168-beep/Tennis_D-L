...
const OUT_JS="data/quant-board.js";
const MODEL_VERSION="TENNIS-EDGE-QUANT-6.1";
const NOW=new Date();
...
function boardBridgeScript(s){
  return `/* Tennis Edge Pro board bridge · ${MODEL_VERSION} */
window.__TEP_BOARD__=${JSON.stringify(s)};
window.__TEP_BOARD_LOADED_AT__=${JSON.stringify(NOW.toISOString())};
try{window.dispatchEvent(new Event("tep-board-ready"))}catch{}
`;
}
async function saveState(s){
  ...
  s.board_integrity={
    checksum:stateChecksum(s),
    generated_at:NOW.toISOString(),
    model_version:s?.meta?.model_version||MODEL_VERSION
  };
  await fs.mkdir(path.dirname(OUT),{recursive:true});
  await fs.writeFile(OUT,JSON.stringify(s,null,2)+"\n","utf8");
  await fs.writeFile(OUT_JS,boardBridgeScript(s),"utf8");
}
...
function selfTest(){
  ...
  const bridge=boardBridgeScript({schema_version:BOARD_SCHEMA,meta:{model_version:MODEL_VERSION},radar:[],upcoming:[],history:[]});
  if(!bridge.includes("window.__TEP_BOARD__=")||!bridge.includes(MODEL_VERSION))throw new Error("SELFTEST_STATIC_BRIDGE");
  ...
  console.log(JSON.stringify({ok:true,model:MODEL_VERSION,schema:BOARD_SCHEMA,tests:["market_no_vig","normal_decision","anomaly_quarantine","cold_fail_closed","stale_quote","dedupe","name_resolver","state_schema","static_board_bridge","walk_forward","rate_limit"]}));
}
