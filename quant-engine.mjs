...
const MODEL_VERSION="TENNIS-EDGE-QUANT-5.3";
const NOW=new Date();
...
async function api(endpoint,params={}){
  ...
  const r=await fetch(u,{
    headers:{"user-agent":"TennisEdgePro/5.3"}
  });
...
async function fetchText(url){
  const r=await fetch(url,{headers:{"user-agent":"TennisEdgePro/5.3"}});
...
async function main(){
  ...
  state.meta={
    ...
    resolution_diagnostics:{
      player_a_unresolved:state.radar.filter(x=>x.pre_status==="DATA GAP"&&x.resolution?.reason==="PLAYER_A_UNRESOLVED").length,
      player_b_unresolved:state.radar.filter(x=>x.pre_status==="DATA GAP"&&x.resolution?.reason==="PLAYER_B_UNRESOLVED").length,
      tour_ambiguous:state.radar.filter(x=>x.pre_status==="DATA GAP"&&x.resolution?.reason==="TOUR_AMBIGUOUS").length,
      unresolved_examples:state.radar.filter(x=>x.pre_status==="DATA GAP").slice(0,8).map(x=>({a:x.player_a,b:x.player_b,t:x.tournament,reason:x.resolution?.reason||"UNKNOWN"}))
    },
    note:"NO BET first. Fail-soft radar: unresolved matches are isolated and never block analyzable ATP/WTA matches. Market calls are reserved for pre-analyzed high-priority candidates. Missing data is never invented."
  };
  await saveState(state);
...
function selfTest(){
  ...
  console.log(JSON.stringify({ok:true,model:MODEL_VERSION,tests:["market_no_vig","name_resolver_multi_format","rate_limit_parser","rate_limit_false_hold_guard"]}));
}
