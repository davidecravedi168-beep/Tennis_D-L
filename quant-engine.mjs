...
const MODEL_VERSION="TENNIS-EDGE-QUANT-5.1";
const NOW=new Date();
...
async function api(endpoint,params={}){
  ...
  const r=await fetch(u,{
    headers:{"user-agent":"TennisEdgePro/5.1"}
  });
...
async function fetchText(url){
  const r=await fetch(url,{headers:{"user-agent":"TennisEdgePro/5.1"}});
...
async function main(){
  ...
  state.meta={
    ...
    note:"NO BET first. Every radar match gets immediate sports pre-analysis; market calls are reserved for the highest-priority candidates. Three-engine ensemble, uncertainty gate, segmented calibration, shadow challenger and rate-aware batching. Missing data is never invented."
  };
  await saveState(state);
...
function selfTest(){
  ...
  console.log(JSON.stringify({ok:true,model:MODEL_VERSION,tests:["market_no_vig","name_resolver","rate_limit_parser","rate_limit_false_hold_guard"]}));
}
