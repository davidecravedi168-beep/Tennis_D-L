...
const MODEL_VERSION="TENNIS-EDGE-QUANT-5.4";
const NOW=new Date();
...
async function api(endpoint,params={}){
  ...
  const r=await fetch(u,{
    headers:{"user-agent":"TennisEdgePro/5.4"}
  });
...
async function fetchText(url){
  const r=await fetch(url,{headers:{"user-agent":"TennisEdgePro/5.4"}});
...
function nearValueCandidates(upcoming){
  return [...(upcoming||[])]
    .filter(x=>x.verdict==="NO BET"||x.verdict==="WATCH")
    .map(x=>({
      event_id:x.event_id,
      player_a:x.player_a,
      player_b:x.player_b,
      ...
      market_age_minutes:x.market_age_minutes,
...
async function main(){
  ...
  state.meta={
    ...
    note:"NO BET first. Decision Funnel attivo: ogni market check registra perché passa o viene scartato. Near VALUE è solo ricerca e non è mai una giocata. Missing data is never invented."
  };
  await saveState(state);
...
function selfTest(){
  ...
  const fx=[
    {verdict:"NO BET",candidate_ev:.08,robust_ev:.03,candidate_edge:.04,robust_edge:.015,confidence:78,data_quality:80,engine_unanimous:true,engine_votes:"3/3",model_disagreement:.04,market_age_minutes:20,no_bet_reasons:["MODELLO_COLD"]},
    {verdict:"NO BET",candidate_ev:-.02,robust_ev:-.05,candidate_edge:-.01,robust_edge:-.03,confidence:60,data_quality:70,engine_unanimous:false,engine_votes:"2/3",model_disagreement:.12,market_age_minutes:30,no_bet_reasons:["EV_NON_POSITIVO"]}
  ];
  const fun=decisionFunnel([{pre_status:"PRE-ANALISI"},{pre_status:"UNRESOLVED"}],fx);
  if(fun.market_checked!==2||fun.positive_ev!==1||fun.analyzable!==1)throw new Error("SELFTEST_FUNNEL");
  const nv=nearValueCandidates(fx);
  if(!nv.length||nv[0].research_score<=nv[1].research_score)throw new Error("SELFTEST_NEAR_VALUE");
  ...
  for(const [x,want] of [["J. Sinner","jannik sinner"],["Sinner J.","jannik sinner"],["Sinner, Jannik","jannik sinner"],["C. Alcaraz","carlos alcaraz"],["De Minaur A.","alex de minaur"]]){
    if(resolveNameKey(x,td)!==want)throw new Error("SELFTEST_NAMES:"+x);
  }
  const fakeHist={
    atp:{...td,ratings:{global:new Map()},ranks:new Map()},
    wta:{names:new Set(),nameResolver:buildNameResolver(new Set()),ratings:{global:new Map()},ranks:new Map()}
  };
  const ev={home:"J. Sinner",away:"C. Alcaraz",league:{name:"ATP US Open"}};
  if(!eventResolution(ev,fakeHist).ok)throw new Error("SELFTEST_FAILSOFT_RESOLUTION");
  const r=parseResetAt("60");if(!r||minutesUntil(r)<0)throw new Error("SELFTEST_RATE");
  const prevRL=state.rate_limit;
  state.rate_limit={remaining:42,reset_at:new Date(Date.now()+3600000).toISOString(),last_status:200};
  if(rateLimitActive())throw new Error("SELFTEST_RATE_FALSE_HOLD");
  state.rate_limit={remaining:0,reset_at:new Date(Date.now()+3600000).toISOString(),last_status:429};
  if(!rateLimitActive())throw new Error("SELFTEST_RATE_TRUE_HOLD");
  state.rate_limit=prevRL;
  console.log(JSON.stringify({ok:true,model:MODEL_VERSION,tests:["market_no_vig","name_resolver_multi_format","rate_limit_parser","rate_limit_false_hold_guard","decision_funnel","near_value_research"]}));
}
