...
const MODEL_VERSION="TENNIS-EDGE-QUANT-5.0";
const NOW=new Date();
...
async function api(endpoint,params={}){
  ...
  const r=await fetch(u,{
    headers:{"user-agent":"TennisEdgePro/5.0"}
  });
...
async function fetchText(url){
  const r=await fetch(url,{headers:{"user-agent":"TennisEdgePro/5.0"}});
...
function selfTest(){
  ...
  console.log(JSON.stringify({ok:true,model:MODEL_VERSION,tests:["market_no_vig","name_resolver","rate_limit_parser"]}));
}
