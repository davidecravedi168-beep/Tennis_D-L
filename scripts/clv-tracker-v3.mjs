...
const MAX_OBS=192; const CLOSING_WINDOW_MIN=180;
...
function sideOdds(p,side){const candidates=side==='A'?[p?.odds_a,p?.price_a,p?.moneyline_a]:[p?.odds_b,p?.price_b,p?.moneyline_b];for(const x of candidates){const n=num(x);if(n>1)return n}return null}
function snapshots(p,board,side){
  const rows=[];
  ...
  const observed_at=p?.current_odds_updated_at||board?.meta?.data_refreshed_at||null,odds=num(p?.current_odds)||sideOdds(p,side);
...
const board=await readJson(BOARD);const ledger=await readJson(LEDGER);if(!board||!ledger)throw new Error('CLV_TRACKER_INPUT_MISSING');let out=await readJson(OUT);if(!out||out.schema!==SCHEMA)out={schema:SCHEMA,created_at:new Date().toISOString(),updated_at:new Date().toISOString(),policy:{forward_only:true,max_observations_per_match:MAX_OBS,closing_window_min:CLOSING_WINDOW_MIN,source:'quant-board observed candidate-side odds',note:'Stores only odds actually observed before scheduled start. No synthetic or retroactive closing prices.'},records:{}};
const upcoming=new Map((board.upcoming||[]).map(p=>[keyFromBoard(p),p]));let captured=0;for(const r of ledger.records||[]){if(stage(r)!=='MARKET_LOCK'||r?.lifecycle?.status!=='OPEN')continue;const p=upcoming.get(keyFromRecord(r));if(!p)continue;const start=iso(r?.immutable?.start_at);if(!start)continue;const k=r.ledger_id||keyFromRecord(r),prev=out.records[k]||{ledger_id:r.ledger_id||null,event_id:String(r.immutable.event_id),model_version:r.immutable.model_version||null,predicted_at:r.immutable.predicted_at||null,start_at:start,candidate_side:r.immutable.candidate_side||null,entry_odds:num(r.immutable.candidate_odds),observations:[]};const before=prev.observations.length;for(const snap of snapshots(p,board,r.immutable.candidate_side)){const observed=iso(snap.observed_at);if(!observed||new Date(observed)>=new Date(start))continue;const age=(new Date(start)-new Date(observed))/60000;if(age>CLOSING_WINDOW_MIN)continue;prev.observations=append(prev.observations,{observed_at:observed,odds:snap.odds,book:snap.book,source:snap.source});}if(prev.observations.length>before)captured+=prev.observations.length-before;out.records[k]=prev}
const values=Object.values(out.records);out.updated_at=new Date().toISOString();out.stats={tracked_matches:values.length,captured_this_run:captured,total_observations:values.reduce((s,r)=>s+(r.observations?.length||0),0),matches_with_prestart_observation:values.filter(r=>(r.observations||[]).length>0).length,matches_with_observation_last_60m:values.filter(r=>(r.observations||[]).some(x=>{const a=(new Date(r.start_at)-new Date(x.observed_at))/60000;return a>=0&&a<=60})).length,latest_observed_at:values.flatMap(r=>r.observations||[]).map(x=>x.observed_at).filter(Boolean).sort().at(-1)||null};await writeJson(OUT,out);console.log(JSON.stringify({ok:true,...out.stats,output:OUT}));
