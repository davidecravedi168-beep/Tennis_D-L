...
function paperStats(rows){
  const eligible=rows.filter(r=>r.lifecycle?.status==="SETTLED"&&["A","B"].includes(r.immutable?.candidate_side)&&num(r.immutable?.candidate_odds)>1&&["A","B"].includes(r.lifecycle?.actual_side));
  const wins=eligible.filter(r=>r.immutable.candidate_side===r.lifecycle.actual_side).length;
  const profit=eligible.reduce((s,r)=>{const won=r.immutable.candidate_side===r.lifecycle.actual_side,odds=num(r.immutable.candidate_odds);return s+(won?odds-1:-1)},0);
  return {n:eligible.length,wins,hit_rate:eligible.length?wins/eligible.length:null,profit_units:profit,roi:eligible.length?profit/eligible.length:null};
}
...
function stats(records){
  ...
  const paper=paperStats(settledMarket),paperWatch=paperStats(settledMarket.filter(r=>r.immutable.verdict==="WATCH")),paperNoBet=paperStats(settledMarket.filter(r=>r.immutable.verdict==="NO BET"));
  const n=eventIds.length,next=n<100?100:n<250?250:n<500?500:1000;
  return {total_forecasts:n,unique_matches:n,ledger_records:records.length,audited_records:active.length,duplicate_records_ignored:records.length-active.length,radar_forecasts:radar.length,market_lock_forecasts:market.length,open_forecasts:n-settled.length,settled_forecasts:settled.length,settled_radar_forecasts:radar.filter(r=>r.lifecycle.status==="SETTLED").length,settled_market_forecasts:settledMarket.length,no_bet_forecasts:market.filter(r=>r.immutable.verdict==="NO BET").length,watch_forecasts:market.filter(r=>r.immutable.verdict==="WATCH").length,qualified_forecasts:market.filter(r=>["VALUE","STRONG VALUE"].includes(r.immutable.verdict)).length,settled_picks:picks.length,wins,hit_rate:picks.length?wins/picks.length:null,profit_units:profit,roi:picks.length?profit/picks.length:null,paper_candidate_settled:paper.n,paper_candidate_wins:paper.wins,paper_candidate_hit_rate:paper.hit_rate,paper_candidate_profit_units:paper.profit_units,paper_candidate_roi:paper.roi,paper_watch_settled:paperWatch.n,paper_watch_wins:paperWatch.wins,paper_watch_hit_rate:paperWatch.hit_rate,paper_watch_profit_units:paperWatch.profit_units,paper_watch_roi:paperWatch.roi,paper_no_bet_settled:paperNoBet.n,paper_no_bet_wins:paperNoBet.wins,paper_no_bet_hit_rate:paperNoBet.hit_rate,paper_no_bet_profit_units:paperNoBet.profit_units,paper_no_bet_roi:paperNoBet.roi,brier:b.length?b.reduce((a,x)=>a+x,0)/b.length:null,log_loss:ll.length?ll.reduce((a,x)=>a+x,0)/ll.length:null,avg_clv:clv.length?clv.reduce((a,x)=>a+x,0)/clv.length:null,clv_sample:clv.length,next_audit_target:next,forecasts_to_next_audit:Math.max(0,next-n)};
}
...
function selfTest(){
  ...
  const ledger={schema:SCHEMA,records:[s,rr,dup]};validate(ledger);const st=stats(ledger.records);if(!(s.lifecycle.brier>0&&s.lifecycle.brier<.2)||st.unique_matches!==2||st.radar_forecasts!==1||st.duplicate_records_ignored!==1||st.paper_candidate_settled!==1||st.paper_candidate_wins!==1||Math.abs(st.paper_candidate_roi-.7)>.000001)throw new Error("LEDGER_SELF_TEST");
  console.log(JSON.stringify({ok:true,schema:SCHEMA,tests:["prematch_lock","radar_capture","immutable_hash","semantic_dedupe","settlement","no_bet_tracking","paper_candidate_tracking","unique_match_stats"]}));
}
