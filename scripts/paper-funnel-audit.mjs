...
function thresholdShadow(rows,tracker,floor){
  const eligible=rows.filter(r=>settled(r)&&isSide(r.immutable?.candidate_side)&&num(r.immutable?.candidate_odds)>=floor&&isSide(r.lifecycle?.actual_side));
  const watchRows=eligible.filter(r=>r.immutable?.verdict==='WATCH');
  const qualifiedRows=eligible.filter(r=>['VALUE','STRONG VALUE'].includes(r.immutable?.verdict));
  return {floor_odds:floor,settled_candidates:eligible.length,watch_count:watchRows.length,qualified_count:qualifiedRows.length,all_candidates:paper(eligible,tracker),watch:paper(watchRows,tracker),official_qualified:paper(qualifiedRows,tracker)};
}
...
const out={schema:'TEP-PAPER-FUNNEL-2',generated_at:now.toISOString(),source:{ledger_schema:ledger.schema||null,ledger_updated_at:ledger.updated_at||null,freshness_minutes:freshnessMinutes,clv_tracker_updated_at:tracker?.updated_at||null},policy:{validation_only:true,thresholds_changed:false,official_pick_logic_changed:false,flat_stake_units:1,strict_nulls:true,clv_definition:'candidate entry odds / best available genuine closing reference - 1',shadow_thresholds:[1.90,1.70,1.65],note:'Observability only: official thresholds and picks remain unchanged. Shadow floors 1.70 and 1.65 collect counterfactual evidence with the same locked model and quality data; they cannot authorize a bet. Missing CLV evidence remains null and never becomes zero.'},funnel:{market_locks:market.length,valid_market_candidates:withCandidate.length,watch:watch.length,no_bet:noBet.length,qualified_value:qualified.length,settled_market_locks:market.filter(settled).length,settled_paper_candidates:paperAll.settled,missing_candidate_side:missingCandidate.length,missing_candidate_odds:missingOdds.length},paper:{all_candidates:paperAll,watch:paperWatch,no_bet:paperNoBet,official_qualified:official},threshold_shadow:[thresholdShadow(market,tracker,1.90),thresholdShadow(market,tracker,1.70),thresholdShadow(market,tracker,1.65)],clv,coverage:{candidate_odds:coverage(market,'candidate_odds'),candidate_ev:coverage(market,'candidate_ev'),candidate_edge:coverage(market,'candidate_edge'),market_consensus:coverage(market,'market_consensus_a'),data_quality:coverage(market,'data_quality'),confidence:coverage(market,'confidence'),avg_data_quality:mean(dq),avg_confidence:mean(conf)},rejection_reasons:reasonCounts(noBet),watch_reasons:reasonCounts(watch),anomalies,status:anomalies.some(x=>x.severity==='problem')?'PROBLEM':anomalies.length?'ATTENTION':'HEALTHY'};

await fs.mkdir(path.dirname(OUT),{recursive:true});
await fs.writeFile(OUT,JSON.stringify(out,null,2)+'\n','utf8');
if(process.argv.includes('--validate')){if(out.schema!=='TEP-PAPER-FUNNEL-2'||!out.funnel||!out.paper||!out.clv||out.policy.strict_nulls!==true||!Array.isArray(out.threshold_shadow)||out.threshold_shadow.length!==3||out.threshold_shadow.some(x=>![1.90,1.70,1.65].includes(x.floor_odds))||!Array.isArray(out.anomalies))throw new Error('PAPER_FUNNEL_INVALID');console.log(JSON.stringify({ok:true,status:out.status,funnel:out.funnel,threshold_shadow:out.threshold_shadow.map(x=>({floor_odds:x.floor_odds,settled_candidates:x.settled_candidates,qualified_count:x.qualified_count,roi:x.all_candidates.roi,clv_sample:x.all_candidates.clv_sample})),clv:out.clv,anomalies:out.anomalies}))}else console.log(JSON.stringify({ok:true,status:out.status,output:OUT,funnel:out.funnel,clv:out.clv,top_rejections:out.rejection_reasons.slice(0,5),anomalies:out.anomalies}));
